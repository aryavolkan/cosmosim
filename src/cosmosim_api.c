    ...
    if (config.quasar) {
        if (config.merger) {
            ...
            generate_quasar_galaxy(
                bodies, n, 0.0, 0.0, (double)n * 2.0, DISK_RADIUS, 0.0, 0.0, config.smbh_mass_frac);
        }
    } else {
        if (config.merger) {
            generate_merger(bodies, n, SEPARATION, APPROACH_VEL);
        } else {
            generate_spiral_galaxy(bodies, n, 0.0, 0.0, GALAXY_MASS, DISK_RADIUS, 0.0, 0.0);
        }
    }
...
COSMOSIM_API void cosmosim_step(SimHandle handle)
{
    ...
    for (int sub = 0; sub < s->config.substeps; sub++) {
        integrator_step(s->bodies,
                        ...
                        s->config.quasar);
        if (s->config.quasar) {
            quasar_step(s->bodies, &s->current_n, s->n_alloc, &s->qcfg, s->config.dt);
        }
    }
