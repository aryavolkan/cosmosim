...
static void accrete_body(Body *smbh, Body *victim)
{
    ...
    if (l_mag > 1e-15) {
        ...
        double s_mag = sqrt(smbh->spin_x * smbh->spin_x + smbh->spin_y * smbh->spin_y +
...
static void
process_accretion(Body *bodies, int n, const QuasarConfig *cfg, double dt, double *mass_swallowed)
{
    (void)dt;
    *mass_swallowed = 0.0;
    ...
    for (int s = 0; s < n; s++) {
        if (bodies[s].type != BODY_SMBH || bodies[s].mass <= 0.0)
            continue;
        Body *smbh = &bodies[s];

        for (int i = 0; i < n; i++) {
            if (i == s || bodies[i].mass <= 0.0)
                continue;
            if (bodies[i].type == BODY_SMBH)
                continue;
...
static void apply_feedback(Body *bodies, int n, const QuasarConfig *cfg)
{
    ...
    for (int s = 0; s < n; s++) {
        ...
        for (int i = 0; i < n; i++) {
            if (i == s || bodies[i].mass <= 0.0 || bodies[i].type == BODY_SMBH)
                continue;
            ...
            if (dist_sq > r_fb_sq || dist_sq < 1e-10)
                continue;
