...
static int alloc_node(OctreeNode *pool, int *pool_size,
                      double center_x, double center_y, double center_z, double half_size)
{
    int idx = (*pool_size)++;
...
static double child_offset(int q, int axis)
{
    ...
    if (axis == 1) return (q & 2) ? -0.5 :  0.5;
    return              (q & 4) ? -0.5 :  0.5;
}

static void insert(OctreeNode *pool, int *pool_size, int node_idx,
...
static void compute_force_on_body(const OctreeNode *pool, int node_idx,
