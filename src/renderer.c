...
static void setup_bloom_fbos(int width, int height)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glFramebufferTexture2D(
            GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, bloom_tex[i], 0);
    }
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    bloom_width = bw;
    bloom_height = bh;
...
static void build_look_at(float *m, float ex, float ey, float ez, float tx, float ty, float tz)
{
                   ...
                   const RendererConfig *rcfg)
{
    ...
    build_look_at(view, eye_x, eye_y, eye_z, cam->target_x, cam->target_y, cam->target_z);
    ...
    if (hdr_active && rcfg && bloom_extract_program && composite_program) {
        ...
        for (int s = 0; s < num_smbh; s++) {
            ...
            float clip_x =
                proj[0] * smv[0] + proj[4] * smv[1] + proj[8] * smv[2] + proj[12] * smv[3];
            float clip_y =
                proj[1] * smv[0] + proj[5] * smv[1] + proj[9] * smv[2] + proj[13] * smv[3];
            float clip_w =
                proj[3] * smv[0] + proj[7] * smv[1] + proj[11] * smv[2] + proj[15] * smv[3];
        ...
        glUniform2fv(
            glGetUniformLocation(composite_program, "u_smbh_screen"), MAX_SMBH, smbh_screens);
        glUniform1fv(glGetUniformLocation(composite_program, "u_smbh_mass"), MAX_SMBH, smbh_masses);
        glUniform1fv(
            glGetUniformLocation(composite_program, "u_lensing_strength"), MAX_SMBH, smbh_lensing);
        glUniform1fv(
...
void renderer_cleanup(void)
{
    ...
    if (bloom_extract_program)
        glDeleteProgram(bloom_extract_program);
    if (bloom_blur_program)
        glDeleteProgram(bloom_blur_program);
